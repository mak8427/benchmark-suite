#!/usr/bin/env python3
import numpy as np, time, os, psutil
n = 18_000
gb = 2*n*n*8 / 1e9
...
dt = time.time() - t0
print(f"GEMM done in {dt:.2f} s → {8*n**3/dt/1e9:.2f} GFLOP/s in time: {time.time() -start_time}", flush=True)


start_time = time.time()
norm = np.linalg.norm(c)
mem  = psutil.virtual_memory().percent
print(f" Frobenius = {norm:.3e}\n  RAM usage = {mem}% in time: {time.time() -start_time}")  #
