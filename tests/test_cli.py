...
def test_add_impl_duplicate(tmp_path, tmp_user_root):
    s = tmp_path / "a.py"; s.write_text("x")
...
def test_add_impl_rejects_invalid(tmp_path, tmp_user_root):
    bad = tmp_path / "notpy.txt"; bad.write_text("x")
    try:
...
def test_add_cli_writes_dir(tmp_path, tmp_user_root, monkeypatch):
    ...
    src = tmp_path / "hello.py"; src.write_text("print('hi')")
...
def test_list_shows_user_modules(tmp_user_root, monkeypatch):
    ...
    d = tmp_user_root / "d"; d.mkdir(); (d/"job_start.sh").write_text("echo")
...
def test_list_shows_builtin(monkeypatch, tmp_path):
    import benchwrap.cli as cli; importlib.reload(cli)
    fake_pkg = tmp_path / "fakepkg"; fake_pkg.mkdir()
...
def test_run_user_dir(tmp_user_root, tmp_path, monkeypatch):
    ...
    empty_pkg = tmp_path / "stdpkg"; empty_pkg.mkdir()
...
def test_run_builtin(monkeypatch):
    import benchwrap.cli as cli; importlib.reload(cli)
    fake_pkg = pathlib.Path.cwd() / "fakepkg"; fake_pkg.mkdir(exist_ok=True)
