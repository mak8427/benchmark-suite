...
"""
        ...
        assert expected_job_dir.exists(), f"Job output directory {expected_job_dir} not found"
        ...
        assert "GFLOP/s" in output_content, "Expected FLOPS calculation not found in output"
        assert "Results ->" in output_content, "Expected results path not found in output"
        assert "Benchmark completed successfully" in output_content, "Success message not found"
    ...
    def test_error_scenarios(self, tmp_path, monkeypatch):
        ...
        def mock_invalid_partition(cmd, **kwargs):
            if cmd[0] == "sbatch":
                ...
                error.stderr = "sbatch: error: Invalid partition name 'invalid-partition'"
                raise error
            return MagicMock(returncode=0)
